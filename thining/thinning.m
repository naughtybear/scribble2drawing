...
function [imgBin] = thinning(filepath)
...
kid_file = dir(filepath) ;
len = length(kid_file) ;
disp(len)
for x = 1 : len
    img = imread(['./input/' kid_file(x).name]) ;
...
%     img = imresize(img,[128 128]);
    ...
    %figure(1), imshow(imgBin,[]);title('OutPut Thinning Image');
    if x < 10
        imwrite(imgBin,['./result/thinning_result/IMG_000' num2str(x) '.png']) ;
    elseif (x >= 10) && (x < 100)
        imwrite(imgBin,['./result/thinning_result/IMG_00' num2str(x) '.png']) ;
    else
        imwrite(imgBin,['./result/thinning_result/IMG_0' num2str(x) '.png']) ;
    end
