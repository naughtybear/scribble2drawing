function finalResultId  = SparseDeep_multi(region,img_num)
...
mkdir(['result\Ranking\SparseDeep_multi\' num2str(img_num)]);
for i = 1:regionNum
    mkdir(['result\Ranking\SparseDeep_multi\' num2str(img_num) '\image_' num2str(i)]);
...
%     alpha_f(i,:)=full(abs(alpha));  %列出所有alpha
    ...
    %% combine sparse和deep
    result_f(i,:) = ((alpha_f(i,:)./sum(alpha_f(i,:))) .* prob_f(i,:));
...
%     a = flipud(a);
%     for j = 1 : 8
%         imwrite(reshape(cal101SilhouettesMaskEdge(a(j,2),:),[50 50]),['result\Ranking\SparseDeep_multi\' num2str(img_num) '\image_' num2str(i)  '\' num2str(j) '.png']);
%     end
%     clear a    
end
%% 第一次跑Sparse，不要註解
