...
%%
% close all;
function [newRegion, newRegionInfo] = selectiveSearch(image)
...
%     fprintf('   http://staff.science.uva.nl/~mark/downloads.html#anigauss\n\n');
    mex SelectiveSearchCodeIJCV/Dependencies/anigaussm/anigauss_mex.c Dependencies/anigaussm/anigauss.c -output anigauss
end
...
colorType = colorTypes{5}; % Single color space for demo

% Here you specify which similarity functions to use in merging
simFunctionHandles = {@SSSimColourTextureSizeFillOrig, @SSSimTextureSizeFill, @SSSimBoxFillOrig, @SSSimSize};
simFunctionHandles = simFunctionHandles(3:4); % Two different merging strategies
...
for i=1:regionNum
    ...
    if (width>20 || height>20)
        newRegion(count,:) = region(i,:);
        newRegionInfo(count,:) = regionInfo(i,:);
        count= count+1;
    end
end
