function result = neural_style(content,style,varargin)
% function result = neural_style(varargin)
...
% Parse the arguments
% Style loss options
% opts.style_image = 'be0700_p_01_03.jpg';
% opts.style_image = styleName;
...
% opts.content_image = '000015.jpg';
% opts.content_image = contentName;
...
opts.gpu = 0;
opts.pooling = 'max';
opts.model_file = 'imagenet-vgg-verydeep-16.mat';
opts.seed = -1;
...
opts.print_iter = 50;
opts.save_iter = 100;
opts.output_image = './result/final/out.png';
...
elseif strcmp(opts.optimizer, 'adam')
    ...
    for t = 1:opts.num_iterations
        [img, ~, optim_state] = adam(objfunc, img, optim_config, optim_state);
...
result = gather(img);
imwrite(result, opts.output_image);
